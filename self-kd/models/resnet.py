...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1_feature_flat = out1_feature.view(x.size(0), -1)
        out2_feature = self.scala2(feature_list[1])
        out2_feature_flat = out2_feature.view(x.size(0), -1)
        out3_feature = self.scala3(feature_list[2])
        out3_feature_flat = out3_feature.view(x.size(0), -1)
        out4_feature = self.scala4(feature_list[3])
        out4_feature_flat = out4_feature.view(x.size(0), -1)

        out1 = self.fc1(out1_feature_flat)
        out2 = self.fc2(out2_feature_flat)
        out3 = self.fc3(out3_feature_flat)
        out4 = self.fc4(out4_feature_flat)
